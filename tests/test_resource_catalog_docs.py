...
class ResourceCatalogDocumentationTests(unittest.TestCase):
    ...
    def test_all_release_resources_have_exact_generated_entries(self):
        scope = _yaml(ROOT / "registry" / "v0.1.yaml")
        ...
        for resource_id in required:
            self.assertEqual(
                1,
                catalog.count(f"<!-- resource:{resource_id} -->"),
                f"Expected exactly one catalog entry for {resource_id}",
            )
