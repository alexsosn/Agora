...
A cold `prepare_corpus` or `load_corpus` must finish inside **one** tool call. Cancellation stops the work and discards the in-flight snapshot export or cold compile (already published snapshots and Git metadata are kept), so a retry repeats that step from its beginning. Agora's generated client configurations therefore give the Context-Fabric server a 90-minute tool-call timeout, longer than the default acquisition and compile limits combined: Claude Code receives a per-server `timeout` (which takes precedence over `MCP_TOOL_TIMEOUT`), and Codex receives `tool_timeout_sec`. If you configure a shorter timeout for this server yourself, or raise `AGORA_CORPUS_ACQUISITION_MAX_MINUTES`, `AGORA_CORPUS_COMPILE_MAX_MINUTES` or `max_compile_minutes` beyond that budget, a cold load of a large corpus such as BHSA may be cancelled every time and never finish.

When the server exits, or (on Linux and macOS) receives SIGTERM or SIGHUP, it stops any Git processes still running for an operation before shutting down. SIGINT does not shut the server down; it cancels in-flight requests, which stops their Git work.
...
Acquisition timeout/cancellation stops the request-owned Git subprocess, including helper processes it started such as the lazy blob fetch of a partial clone, before the operation returns and releases the repository lock. Helper cleanup is regression-tested on Linux; macOS uses the same POSIX process-group mechanism but is not covered by automated tests; on Windows Agora asks `taskkill /T` to stop the process tree, which is best-effort and not covered by automated tests. Temporary snapshot-export trees are cleaned by the existing materialization `finally` path; published immutable snapshots are replaced atomically only after validation. A retry therefore reuses complete published state or starts materialization again rather than treating a partial temporary tree as a valid corpus.
