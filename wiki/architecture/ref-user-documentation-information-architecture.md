...
Paths below start at the repository README. `Current` describes links actually available now; `Target` names *future roles* with implementing tickets, not extant hyperlinks.

| User question / journey | Current shortest discoverable path and gap | Target shortest path and success test |
|---|---|---|
| Does Agora have a resource for my research question? | README → plugin chooser, then inspect canonical `registry/resources.yaml` or query an installed plugin to confirm a specific corpus such as ETCBC Targum. No human resource catalog is linked. | README → user guide (#165) → generated resource catalog (#167) → resource entry with provider/plugin and scope. Answer presence and what to install without YAML. |
| Which plugin/resource do I choose? | README → “Choosing a plugin” → installation; four families are distinguishable, but corpus members and per-plugin constraints require other files. | README → user guide → plugin chooser (#165/#166) or catalog (#167) → plugin detail (#168) → install. Show local/remote, resource, license/provenance and caveats *before* installation. |
| Can I install and reach one successful operation? | README → installation guide → example prompt; no bounded, step-by-step validated first-success tutorial for each family. | README → user guide → installation how-to (#172, retaining #150 facts) → family tutorial (#170). The tutorial includes prerequisites, exact bounded task, expected result and next step. |
