...
Idempotent reuse recomputes source and runtime hashes and re-reads distribution metadata. Editing plugin source, installed dependency files, distribution metadata, the execution manifest, or the pip report invalidates the installation. `--repair` uses a staging environment and replacement/rollback path.

Per-source and per-environment installation locks use a one-way migration protocol on the historical lock pathname. When no lock file exists, current Agora creates it atomically with `O_EXCL` and writes the fixed marker `agora-materializer-lock-v3`. That create race is the compatibility boundary with pre-#62 sentinel-based checkouts: if an older holder created the path first, current code waits for it to release and unlink; if current code creates the marker first, pre-#62 code remains excluded by pathname existence.

The marked lock file is persistent modern protocol state and is not removed on normal release or after a crash. Current Agora and the advisory-only implementation introduced by #62 both take an OS-backed exclusive advisory lock on that same historical pathname, so those generations exclude each other. Process death releases OS lock ownership automatically while leaving the marker intact for safe reuse. Avoiding automatic unlink also avoids replacing an inode that a #62 waiter may already have open, and works consistently on Windows where deleting an open/locked file is not a portable operation.

The marker has one newline-free ASCII byte representation on every supported platform. Before advisory acquisition current Agora uses pathname metadata only; a marker-sized candidate is opened and locked with the remaining timeout, then its exact bytes and filesystem identity are verified through the acquired readable handle before the critical section starts. This ordering is required on Windows because exclusive locks are mandatory there: opening a second handle to read locked marker bytes before ownership can otherwise block outside Agora's timeout machinery.

An empty or otherwise unrecognized historical lock file is deliberately treated as ambiguous: it may be stale pre-v3 residue, but it may also belong to a live pre-#62 installer, which wrote no owner metadata and took no advisory lock. Agora therefore never deletes or rewrites such a file automatically. If the installer reports a legacy/pre-migration lock that remains after its wait timeout, first confirm that no older materializer operation is running; only then remove that reported file once and retry so current Agora can establish the v3 marker.

The migration is intentionally one-way during normal operation. A pre-#62 checkout cannot install into a root after the v3 marker has been established. If a deliberate downgrade is required, first confirm that no #62 or current materializer operation is running, then remove the marked lock file manually before invoking the pre-#62 checkout. Never remove a marked v3 lock during ordinary modern use.
