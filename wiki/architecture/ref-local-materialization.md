...
Per-source and per-environment installation locks use a migration-safe hybrid protocol. Current Agora versions serialize through an OS-backed advisory sidecar, while also creating the historical visible `O_EXCL` sentinel for the duration of the critical section. That visible sentinel keeps pre-#62 checkouts out; current clean releases remove it so older checkouts are not permanently wedged. If a current process dies, the OS releases the sidecar and the marked visible sentinel lets a later current process recognize and recover its crash residue.

An empty or otherwise unrecognized historical lock file is deliberately treated as ambiguous: it may be stale pre-#63 residue, but it may also belong to a live pre-#62 installer, which wrote no owner metadata and took no advisory lock. Agora therefore never deletes such a file automatically. If the installer reports a legacy/pre-migration lock that remains after its wait timeout, first confirm that no older materializer operation is running; only then remove that reported legacy lock file once and retry. Do not delete a lock file while an older checkout may still be active.
