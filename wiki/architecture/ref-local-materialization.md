...
Per-source and per-environment installation locks use a three-part migration bridge. Current Agora versions first serialize with an OS-backed advisory sidecar. They then create the historical visible path with `O_EXCL`, which keeps pre-#62 sentinel-based checkouts out, and also hold an advisory lock on that same historical visible path, which keeps the advisory-only implementation introduced by #62 out. Current code enters the mutation critical section only after all three conditions are established.

Current clean release removes the historical visible sentinel while the advisory locks are still held, then releases the historical advisory lock and the sidecar. If a current process dies, OS lock ownership is released automatically. Its marked visible sentinel can be reused by a later current process only after that process acquires both the current sidecar and the historical-path advisory lock; this prevents crash recovery from bypassing a #62 process that attached to the marked file.

An empty or otherwise unrecognized historical lock file is deliberately treated as ambiguous: it may be stale pre-#63 residue, but it may also belong to a live pre-#62 installer, which wrote no owner metadata and took no advisory lock. Agora therefore never deletes such a file automatically. If the installer reports a legacy/pre-migration lock that remains after its wait timeout, first confirm that no older materializer operation is running; only then remove that reported historical lock file once and retry. Do not delete a lock file while an older checkout may still be active.
